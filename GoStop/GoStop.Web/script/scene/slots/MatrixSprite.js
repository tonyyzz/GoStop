...
cc.Class({
    ...
    init: function (slotsMgr, row, column) {
        ...
        if (Global.gameID === 5) {
            this.maxRow = 3;    //区分4*5和3*5
            this.wildLab = cc.instantiate(this.slotsManager.wildLab).getComponent(cc.Label);
            this.node.addChild(this.wildLab.node);
            this.wildLab.node.active = false;
        } else
            this.maxRow = 2;

        this.sprite = this.node.getComponent(cc.Sprite);
        this.changeSprite();
        this.runStartPos = this.slotsManager.matrixTwo[this.maxRow + 1][0].node.getPositionY();
        this.endPos = this.slotsManager.matrixTwo[0][0].node.getPositionY() - this.node.height * (this.maxRow + 1);
    ...
    //// ---《富兰克林》主题
    // 显示wild奖励金额
    showWildGold: function (gold) {
        this.wildLab.string = gold;
        this.wildLab.node.active = true;
    },
    closeWildGold: function () {
        this.wildLab.node.active = false;
    },
    /////////////////////
    ...
    update: function (dt) {
        if (this.isMove) {
            ...
            if (this.isRollOver && this.node.getPositionY() <= this.startPosY && this.node.getPositionY() >= this.startPosY - (this.speed + 5)) {
                ...
                if (this.row <= this.maxRow) {    //行下标小于等于最大行的情况下
